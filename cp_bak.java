...
//                }
            ...
            this.populations[pop][0].setWeight(this.P.moweight);
    ...
    public void run() {
        ...
        try {
            ...
            while (this.hasToContinue(this.step, currentTime, endTime, noMREChangeStop)) {
                ...
                if(this.step == 0){
                    evaluationsToMonitor.put("Best solution", 99999.99999);
                }else {
                ...
                if(this.step == 0){
                    (this.bestSolution = this.P.dataset.generateTree(this.P.outgroup, Parameters.StartingTreeGeneration.RANDOM, this.P.startingTreeGenerationRange, this.P.startingTreeModel, this.P.startingTreeDistribution, this.P.startingTreeDistributionShape, this.P.startingTreePInv, this.P.startingTreePInvPi, this.P, this.monitor)).setName("Consensus pruning best solution");
                    this.bestSolution.clone(this.populations[0][0]);
                    this.bestSolution.setParsimonyValue(999999);
                    this.bestSolution.setLikelihoodValue(999999);
                }
    ...
    private class IterateGA implements Runnable
    {
        ...
        public void run() {
            try {
                ...
                this.parent = new Tree[ConsensusPruning.this.indNum];
                for(int ind=0;ind < ConsensusPruning.this.indNum;ind++){
                    this.parent[ind] = ConsensusPruning.this.P.dataset.generateTree(ConsensusPruning.this.P.outgroup, Parameters.StartingTreeGeneration.RANDOM, ConsensusPruning.this.P.startingTreeGenerationRange, ConsensusPruning.this.P.startingTreeModel, ConsensusPruning.this.P.startingTreeDistribution, ConsensusPruning.this.P.startingTreeDistributionShape, ConsensusPruning.this.P.startingTreePInv, ConsensusPruning.this.P.startingTreePInvPi, ConsensusPruning.this.P, ConsensusPruning.this.monitor);
                    this.parent[ind].clone(this.population[ind]);
                    //this.parent[ind] = this.population[ind].clone();
                }
                ...
                for(int ind = 0 ;ind < ConsensusPruning.this.indNum;ind++){
                    int best = ind;
                    for(int index=ind+1 ;index< ConsensusPruning.this.indNum*2;index++) {
                        if (index >= ConsensusPruning.this.indNum && best >= ConsensusPruning.this.indNum) {
                            if (this.parent[index % ConsensusPruning.this.indNum].isBetterThan(this.parent[best % ConsensusPruning.this.indNum])) {
                                best = index;
                            }
                        } else if (index >= ConsensusPruning.this.indNum && best < ConsensusPruning.this.indNum){
                            if (this.parent[index % ConsensusPruning.this.indNum].isBetterThan(this.population[best])) {
                                best = index;
                            }
                        }else{
                            if (this.population[index].isBetterThan(this.population[best])) {
                                best = index;
                            }
                        }
                    }
                    if(best >= ConsensusPruning.this.indNum){
                        this.offspring[ind].clone(this.parent[best % ConsensusPruning.this.indNum]);
                    }else{
                        this.offspring[ind].clone(this.population[best]);
                    }
                }
                for(int ind = 0 ;ind < ConsensusPruning.this.indNum;ind++){
                    this.population[ind].clone(this.offspring[ind]);
                }
